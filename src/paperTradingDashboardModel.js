...
export function buildPaperTradingDashboardModel(runtimeState = {}) {
  ...
  return {
    ...
    account: {
      initialCapital: n(snapshot.initialCapital),
      cash: n(snapshot.cash),
      equity: n(snapshot.equity),
      realizedPnL: n(snapshot.realizedPnL),
      unrealizedPnL: n(snapshot.unrealizedPnL),
    ...
    risk: {
      grossExposure: n(snapshot.grossExposure),
      capitalUtilizationPercent: pct(snapshot.capitalUtilizationPercent),
      dailyLossPercent: pct(snapshot.risk?.dailyLossPercent),
      safe: snapshot.risk?.safe === true,
    ...
    positions: {
      open: positions.filter(p => p.status === 'OPEN').map(p => ({
        ...
        markPrice: n(p.markPrice ?? p.entryPrice),
